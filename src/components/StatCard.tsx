...
export function StatCard({
  ...
  return (
    <div className={cn("rounded-2xl p-5 transition-smooth hover:-translate-y-0.5 animate-scale-in", styles[currentVariant])}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className={cn("text-sm font-medium opacity-80", (variant === "balance" || isNegativeBalance) ? "text-primary-foreground/90" : "text-muted-foreground")}>{label}</div>
          <div className="font-display text-2xl sm:text-3xl font-bold mt-1.5 tracking-tight truncate">{formatINR(value)}</div>
          {hint && <div className={cn("text-xs mt-1.5", (variant === "balance" || isNegativeBalance) ? "text-primary-foreground/80" : "text-muted-foreground")}>{hint}</div>}
        </div>
        <div className={cn("h-11 w-11 rounded-xl flex items-center justify-center shrink-0", iconBg[currentVariant])}>
